...
typedef std::vector<proslam::FramePoint> FramepointVector;
typedef std::vector<proslam::Frame> FrameVector;
...
void PoseOptimizer::Initialize(proslam::Frame* curr_frame_ptr,proslam::Frame* prev_frame_ptr){
...
void PoseOptimizer::ComputeError(proslam::FramePoint* fp){
    /**
    ...
    */
    ...
    p_caml = T_prev2curr*fp->previous()->cameraCoordinatesLeft();
    p_camr = T_prev2curr*parameters.T_caml2camr.inverse()*fp->previous()->cameraCoordinatesLeft();

    if (fp->previous()->landmark()){
        // We first convert the landmark world coordinates to previous camera coordinates
        //Eigen::Vector3d p_caml_prev;
    ...
    lcam_pixels = current_frame_ptr->cameraLeft()->cameraMatrix() * p_caml;
    lcam_pixels[0] = lcam_pixels[0]/lcam_pixels[2];
    lcam_pixels[1] = lcam_pixels[1]/lcam_pixels[2];
    
    rcam_pixels = current_frame_ptr->cameraRight()->cameraMatrix() * p_camr;
    ...
    reproj_error[0] = lcam_pixels[0] - fp->keypointLeft().pt.x;
    reproj_error[1] = lcam_pixels[1] - fp->keypointLeft().pt.y;

    reproj_error[2] = rcam_pixels[0] - fp->keypointRight().pt.x;
    reproj_error[3] = rcam_pixels[1] - fp->keypointRight().pt.y;
...
void PoseOptimizer::Linearize(proslam::FramePoint* fp){
    /**
    ...
    double translation_factor = std::min(parameters.maximum_reliable_depth/p_caml[2],double(1.0));

    // Calculate the jacobian
    const proslam::Camera* cameraLeft = current_frame_ptr->cameraLeft();
    const proslam::Camera* cameraRight = current_frame_ptr->cameraRight();
    Eigen::Matrix<double,4,6> J = FindJacobian(p_caml,p_camr,cameraLeft,cameraRight,translation_factor);
...
Eigen::Matrix<double,4,6> PoseOptimizer::FindJacobian(Eigen::Vector3d& left_cam_coordinates,Eigen::Vector3d& right_cam_coordinates,const proslam::Camera* camera_l,const proslam::Camera* camera_r,double omega){
    ...
    fx_l = camera_l->cameraMatrix()(0,0);
    fy_l = camera_l->cameraMatrix()(1,1);

    fx_r = camera_r->cameraMatrix()(0,0);
    fy_r = camera_r->cameraMatrix()(1,1);
    ...
    J_Transform.block<3,3>(0,0) = identity3.cast<double>() * omega;
    J_Transform.block<3,3>(0,3) = -2*hat_cam_coordinates.cast<double>();
...
void PoseOptimizer::Solve(){
    ...
    double damping_factor = measurements * 5;
...
void PoseOptimizer::Update(){
    ...
    //for(unsigned int i =0; i<current_frame_ptr->points.size();i++){
    //    proslam::FramePoint* fp = current_frame_ptr->points[i].get();
    //    
    //    fp->setWorldCoordinates(current_frame_ptr->T_world2cam * fp->cameraCoordinatesLeft()); 
    //};
...
void PoseOptimizer::OptimizeOnce(){
    ...
    for(unsigned int i =0; i < current_frame_ptr->points().size(); i++){
        proslam::FramePoint* fp_ptr = current_frame_ptr->points()[i];
        ComputeError(fp_ptr);
        Linearize(fp_ptr);
    };
    Solve();
...
void PoseOptimizer::VisualizeFramepoints(FramepointVector fp_vec,cv::Mat& image,int cam,cv::Scalar color = cv::Scalar(0,0,255)){
    ...
    for(proslam::FramePoint& fp : fp_vec){
        cv::KeyPoint keypoint;
        if(cam == 0){
            keypoint = fp.keypointLeft();
        }
        else if (cam == 1)
        {
            keypoint = fp.keypointRight();
        }
...
void PoseOptimizer::VisualizeFramepointComparision(FramepointVector fp_vec1,cv::Mat& image_1,FramepointVector fp_vec2,cv::Mat& image_2){
    ...
    for(proslam::FramePoint& fp : fp_vec1){
        cv::KeyPoint keypoint;
        keypoint = fp.keypointLeft();
        cv::circle(img_rgb1,keypoint.pt,3,cv::Scalar(0,0,255),CV_FILLED);
    };
    for(proslam::FramePoint& fp : fp_vec2){
        cv::KeyPoint keypoint;
        keypoint = fp.keypointLeft();
